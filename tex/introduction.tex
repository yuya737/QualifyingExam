...
\section{Motivation}
%
Geospatial data---data imbued with a reference to a location on the Earth's surface---is ubiquitous, underlying many of the most pressing scientific problems and societal issues of our time, from public health~\cite{zhang_visualization_2023} to climate change~\cite{eyring2016overview}.
%
Decisions about where to allocate water in a drought-stricken region, how a community should prepare for the next flood, how we adapt to climate change impacts as they unfold are, at their core, made against a backdrop of geospatial data.
...
%
A single phenomenon is now routinely captured across many interacting variables, many simulation instances, and many heterogeneous data modalities, each adding a distinct dimension of complexity that an analyst must contend with before any insight can be drawn and communicated.
%
...
%
I borrow this notion from Kehrer and Hauser, whose comprehensive survey of \emph{multifaceted} scientific data~\cite{kehrer_visualization_2013} formalizes a taxonomy of five facets: spatiotemporal, multivariate, multimodal, multirun, and multimodel.
%
Geospatial data, I argue, is no exception, exhibiting several of these facets at once: it is frequently multivariate~\cite{strode2020bivariate}, ensemble-valued~\cite{kehrer2013visualization}, and multimodal~\cite{wu_geosptial_2024}, often simultaneously.
%
As Kehrer and Hauser observe, and as the community has long recognized, no single, one-size-fits-all solution serves every facet; each application domain and facet demands its own treatment.
%
...
This challenge, however, is far from new. 
%
The task of analzing and communicating insights from geospatial data long predates the modern field of \emph{visualization}, with deep roots in cartography, which has for centuries studied the varied ways to analyze and communicate geospatial data, often on a map~\cite{bertin1983semiology, Tyner_2014}.
%
...
%
In his seminal book \emph{The Visual Display of Quantitative Information}, Tufte presents numerous such examples, including Charles Joseph Minard’s flow map of Napoleon’s 1812 Russian campaign, a staple of introductory visualization classes for its creativity and efficacy~\cite{tufte2001visual}.

Yet increasingly, these techniques fall short: a static map struggles to convey multiple interacting variables, the spread across an ensemble of model runs, or the output of a predictive model that fuses many data modalities.
%
This modeling turn has only accelerated with Geospatial AI (GeoAI): geospatial foundation models such as Google's AlphaEarth~\cite{brown_alphaearth_2025} and NASA's Prithvi~\cite{szwarcman2024geospatial} distill vast Earth-observation archives into information-rich embeddings that show great promise for geospatial applications~\cite{bell_earth_2026}, yet arrive as opaque feature vectors raising pressing questions of interpretability.
%
The rise of these foundation models only compounds the challenge, adding complexity that the standard methods of the domain were never built to handle natively.
%
It is precisely this gap---between the complexity of modern geospatial data and the expressiveness of classical cartography---that necessitates \emph{visual analytics}.
%
This proposal aims to build on the analysis methods already established in each application domain, while identifying where novel methods can contribute and what they can offer.
%
The central question I seek to address in this proposal is thus:

\begin{quote}
\emph{In the face of the ever-increasing complexity of geospatial data along its many facets, how can we support its analysis? What role can Visual Analytics play?}
\end{quote}
\noindent
...
%
For each of the three facets introduced above, I now expand on what makes it difficult to analyze and where existing approaches fall short, framing in each case the open challenge that the corresponding chapter takes up.
%
...
%
Padilla suggests that visualization comprehension can be understood as a process of matching the visual description of the visualization to the graph schema~\cite{padilla_case_2018,padilla_decision_2018}.
%
However, the graph schema of maps is particularly strict relative to other visualizations. 
%
For instance, viewers expect that the spatial position of a mark corresponds to its geographic location, and any height above the map plane in a 3D visualization is immediately read as altitude.
%
...
%
The open challenge, then, is to make the full distribution of ensemble behavior explorable and comparable---and to support claims drawn from it that are statistically grounded rather than merely apparent to the eye.
...
%
These sources, from satellite imagery and terrain descriptors to hydrometeorological time series and sparse observational records, differ in resolution, modality, and reliability, so combining them into a coherent predictive signal is itself a substantial modeling challenge.
%
The difficulty is sharpened by supervision since the prediction targets of greatest interest are often observed only sparsely and indirectly, so a model must learn from noisy, uncertain proxies rather than clean ground truth.
%
...
%
This exposes a deeper tension. 
%
As predictive accuracy improves, interpretability tends to degrade, which is directly at odds with the understanding that analysts and decision-makers need in order to trust and act on a forecast.
%
The usual recourse is post-hoc attribution.
%
Saliency maps or feature-importance scores are often applied after training, but such methods explain individual predictions rather than revealing the structure a model has actually learned~\cite{rudin_stop_2019}.
%
The open challenge, then, is twofold: to fuse complementary modalities into an accurate predictive model, and to keep its reasoning legible \emph{by design}---interpretable enough for analysts to interrogate, trust, and ultimately act on, rather than explained away after the fact.
...
\begin{itemize}[topsep=2pt, leftmargin=*]
    \item \textbf{Chapter~\ref{chp:hextiles}} develops the multivariate-structure thread. I introduce HexTiles, a domain-agnostic hexagonal-tiling encoding that combines semantic icons with hexagonal tessellation to render several variables and the interactions among them on a single map while preserving its spatial schema, and that stays faithful across data of differing spatial discretizations and qualities. By aggregating to a shared tessellation, HexTiles directly confronts the
        Modifiable Areal Unit Problem, surfacing within-tile variability through confidence encodings rather than hiding it. I evaluate the design through case studies in California water management and Texas presidential-election data, with a controlled user study, qualitative feedback, and an expert review with ecologists and hydrologists.
    ...
    \item \textbf{Section~\ref{sec:agu}} (within the ClimateSOM chapter) turns that workflow from exploration to inference. I extend ClimateSOM with a permutation-based hypothesis test built on the Earth Mover's Distance, together with a within-year coherence test and a clustering-based decomposition that attributes shifts to reorganization among identifiable pattern clusters. Applying it to the co-variability between California's Sacramento--San Joaquin Bay-Delta and broader U.S. West Coast
        precipitation in LOCA2-downscaled CMIP6 (15 models, three emission scenarios), I find that the historical conditioning between the two regions departs significantly from its past behavior, a ``decoherence'' that strengthens under higher emissions and longer horizons and is most pronounced in spring.
        \end{itemize}
    \item \textbf{Chapter~\ref{chp:deluge}} develops the multimodality-and-predictive-modeling thread. I present DELUGE, to my knowledge the first end-to-end learned system for daily, ${\sim}1$\,km insured pluvial flood-damage prediction across the highest-claim regions of CONUS as defined by National Flood Insurance Program (NFIP) claims. A multimodal convolutional model trained on NFIP insurance claims, DELUGE routes Earth-observation foundation-model embeddings (AlphaEarth) and terrain descriptors through physics-shaped Value and Temporal Modulators
        whose learned parameters are themselves hydrologically meaningful, yielding interpretability by design. It outperforms tuned tree-based baselines, with the largest gains on the high-cost claims that matter most. I consider this the base predictive model for a visual analytics system that will allow stakeholders, such as insurers and emergency managers, to understand DELUGE's pluvial flood-damage predictions and interrogate its reasoning.
    \item \textbf{Chapter~\ref{chp:future} --- Ongoing and Future Research Plans} consolidates the HexTiles work with a larger and more diverse user study, then outlines three directions that carry the dissertation forward before concluding. The first is a visual analytics system built on DELUGE that lets stakeholders inspect \emph{why} a damage prediction is made, attributing it along the hazard, exposure, and vulnerability triad that structures disaster risk, and pose counterfactual what-if queries to the model. The second generalizes this interpretive goal beyond any single model, proposing a visual analytics system that reveals how a trained model uses the embeddings it draws from a geospatial foundation model, grounding that reliance in human-understandable spatial concepts and surfacing the predictive structure it uses that no known concept explains. The third completes the ClimateSOM extension by folding its inferential and interpretive capability back into the interactive workflow, and develops a separate visual analysis workflow for studying how the spatial co-occurrence of extreme events shifts in location, intensifies, or weakens under a changing climate. Taken together, this dissertation advances visual analytics as a tool for complex geospatial analysis, with a particular focus on imminent environmental challenges.
\end{itemize}
